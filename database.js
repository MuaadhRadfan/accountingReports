...
export class Database {
     ...
     * @param {{ type: 'put'|'add'|'delete', store: string, value?: object, id?: IDBValidKey }[]} ops
     */
    ...
    async getNextSequence(prefix) {
        const metaKey = `seq_${prefix}`;
        const current = parseInt((await this.getMeta(metaKey)) || '0', 10);
        const next = current + 1;
        await this.setMeta(metaKey, String(next));
        return next;
    }

    async setSequence(prefix, value) {
        const metaKey = `seq_${prefix}`;
        await this.setMeta(metaKey, String(value));
    }

    async getCurrentSequence(prefix) {
        return parseInt((await this.getMeta(`seq_${prefix}`)) || '0', 10);
    }
}
