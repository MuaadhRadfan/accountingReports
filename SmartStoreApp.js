...
import {
    ...
    buildPaymentVoucherJournal,
    entryPartyRunningDelta,
    buildReversalJournal,
    journalLinesToAccountEntries
...
/** WAC: reverse a purchase line from product state (used when editing purchases). */
function applyPurchaseLineReverseToProduct(product, item) {
    const returnedPieces = item.quantity;
    const newStock = product.stock - returnedPieces;
    if (newStock < 0) {
        throw new Error(`مخزون سالب غير مسموح: ${product.name}`);
    }
    const oldTotalCost = product.cost_price * product.stock;
    const returnedCost = item.unit_cost * returnedPieces;
    const newTotalQty = product.stock - returnedPieces;
    const newWacCost = newTotalQty > 0 ? (oldTotalCost - returnedCost) / newTotalQty : product.cost_price;
    return { ...product, stock: newStock, cost_price: newWacCost };
}

function applyPurchaseLineAddToProduct(product, item) {
    const qtyInPieces = item.quantity;
    const newStock = product.stock + qtyInPieces;
    const oldTotalCost = product.cost_price * product.stock;
    const newTotalCost = oldTotalCost + (item.unit_cost * qtyInPieces);
    const newTotalQty = product.stock + qtyInPieces;
    const newWacCost = newTotalQty > 0 ? newTotalCost / newTotalQty : 0;
    return { ...product, stock: newStock, cost_price: newWacCost };
}

/**
 * Reversal of the original purchase journal (when old GL lines are missing or for fallback).
 * Credits inventory, debits cash and/or AP as appropriate.
 */
function buildPurchaseReversalFromInvoiceData(purchase, journalId, now) {
    const total = Number(purchase.total) || 0;
    const pm = purchase.payment_method;
    const lines = [
        {
            account_code: AC.INV,
            debit: 0,
            credit: total,
            entry_kind: 'purchase_return_inv',
            memo: `عكس مخزون — مرتجع ${purchase.purchase_number}`
        }
    ];
    if (pm === 'cash' || pm === 'mixed') {
        const paid = pm === 'cash' ? total : (Number(purchase.paid_amount) || 0);
        lines.push({
            account_code: AC.CASH,
            debit: paid,
            credit: 0,
            entry_kind: 'purchase_return_cash',
            memo: `عكس نقدي — مرتجع ${purchase.purchase_number}`
        });
    }
    if (purchase.supplier_id && (pm === 'credit' || pm === 'mixed')) {
        const due = pm === 'credit' ? total : (total - (Number(purchase.paid_amount) || 0));
        lines.push({
            account_code: AC.AP,
            debit: due,
            credit: 0,
            party_key: `supplier:${purchase.supplier_id}`,
            entry_kind: 'purchase_return_ap',
            memo: `عكس مستحق — مرتجع ${purchase.purchase_number}`
        });
    }
    assertBalanced(lines);
    const refId = String(purchase.purchase_number);
    return journalLinesToAccountEntries(
        journalId,
        lines,
        'purchase_reversal',
        refId,
        `purchase_reversal:${refId}`,
        now
    );
}
...
export class SmartStoreApp {
    constructor() {
     ...
     async init() {
         try {
             ...
             await this.ensureOpeningBalanceEntry();
    ...
    async seedDataIfNeeded() {
        ...
        if (products.length === 0) {
         ...
         if (users.length === 0) {
             const defaultUsers = [
                 ...
                 { username: 'developer', password: await hashPassword('dev123'), role: 'developer', name: 'مطور النظام', active: true }
             ];
     ...
     async ensureOpeningBalanceEntry() {
         ...
         const openingEntries = [
             {
                 ...
                 ref_id: 'OPENING',
                 ref_key: 'opening_cash',
                 entry_kind: 'opening_cash',
                 memo: 'رصيد نقدي افتتاحي',
                 created_at: now
             },
             {
                 journal_id: journalId,
                 account_code: AC.EQUITY,
                 debit: 0,
                 credit: openingBalance,
                 ref_type: 'opening_balance',
                 ref_id: 'OPENING',
                 ref_key: 'opening_equity',
                 entry_kind: 'opening_equity',
                 memo: 'رأس مال افتتاحي',
                 created_at: now
             }
         ];
         await this.db.executeWrites(openingEntries.map(e => ({ type: 'add', store: 'account_entries', value: e })));
     }
      ...
      async loadSettings() {
         try {
             ...
             const taxId = await this.getSetting('tax_id', '');
             const currency = await this.getSetting('currency', 'YER');
             const branch = await this.getSetting('branch', 'الفرع الرئيسي');
             ...
             if (el('setting-tax-id')) el('setting-tax-id').value = taxId;
             if (el('setting-currency')) el('setting-currency').value = currency;
             if (el('setting-branch')) el('setting-branch').value = branch;
     ...
     async saveSettings() {
         ...
         const taxId = document.getElementById('setting-tax-id').value;
         const currency = document.getElementById('setting-currency').value;
         const branch = document.getElementById('setting-branch').value;
         ...
         await this.setSetting('tax_id', taxId);
         await this.setSetting('currency', currency);
         await this.setSetting('branch', branch);
    ...
    hasPermission(permission) {
     ...
     canAccess(screen) {
         ...
         if (role === ROLES.ADMIN || role === ROLES.DEVELOPER) return true;
         if (role === ROLES.MANAGER) {
             return ['dashboard', 'pos', 'purchases', 'inventory', 'expenses', 'accounting', 'activity'].includes(screen);
         }
    ...
    bindEvents() {
        ...
        document.getElementById('product-form').addEventListener('submit', (e) => {
         ...
         document.getElementById('add-purchase-btn').addEventListener('click', () => this.openPurchaseModal());
         document.getElementById('purchase-form').addEventListener('submit', (e) => {
             e.preventDefault();
             this.savePurchase();
         });
         document.getElementById('close-purchase-modal').addEventListener('click', () => this.closePurchaseModal());
         document.getElementById('add-purchase-item').addEventListener('click', () => this.addPurchaseItemRow());
         document.getElementById('purchase-items-container').addEventListener('click', (e) => {
             if (e.target.closest('.remove-purchase-item')) {
         ...
         document.getElementById('purchase-payment-method').addEventListener('change', (e) => {
             const paidGroup = document.getElementById('purchase-paid-group');
             if (paidGroup) {
                 paidGroup.style.display = e.target.value === 'cash' || e.target.value === 'mixed' ? 'block' : 'none';
             }
             this.calculatePurchaseTotal();
         });
         document.querySelectorAll('#purchase-items-container input').forEach(input => {
             input.addEventListener('input', () => this.calculatePurchaseTotal());
         });
        ...
        document.getElementById('report-date-to')?.addEventListener('change', () => this.refreshReportFilters());
         ...
         document.getElementById('reports-screen')?.addEventListener('click', (e) => {
             if (e.target.closest('.report-print-btn')) {
                 e.preventDefault();
                 void this.printActiveReportWindow().catch((err) => console.error(err));
             }
         });
    ...
    bindDelegatedUi() {
        ...
        document.getElementById('users-table')?.addEventListener('click', (e) => {
         ...
         document.getElementById('purchases-table')?.addEventListener('click', (e) => {
             const b = e.target.closest('[data-purchase-action]');
             if (!b) return;
             const id = parseInt(b.dataset.id, 10);
             const number = b.dataset.number;
             if (b.dataset.purchaseAction === 'edit') this.openPurchaseModal(id);
             if (b.dataset.purchaseAction === 'return') this.handlePurchaseReturn(id, number);
             if (b.dataset.purchaseAction === 'print') this.printPurchaseInvoice(id);
         });
     }
    ...
    async loadDashboard() {
        try {
            ...
            const todayCogs = todayInvoices.reduce((sum, i) => {
                const c = Number(i.cost) || 0;
                if (i.type === 'sale') return sum + c;
                if (i.type === 'return') return sum - c;
                return sum;
            }, 0);
            const openingBalance = parseFloat(await this.getSetting('opening_balance', '0')) || 0;
            ...
            const cogsEl = document.getElementById('today-cogs');
            if (cogsEl) cogsEl.textContent = this.formatCurrency(todayCogs);
    ...
    getDisplayStock(product) {
     ...
     async searchProducts(query) {
         ...
         this.searchTimeout = setTimeout(async () => {
             ...
             // If query matches a barcode exactly, auto-add to cart
             if (query.length >= 4) {
                 const exactMatch = products.find(p => p.barcode && p.barcode.toLowerCase() === q);
                 if (exactMatch) {
                     await this.addToCart(exactMatch.id);
                     document.getElementById('pos-search').value = '';
                     this.renderProducts(products); // refresh to show updated stock
                 }
             }
         }, 200);
     }
    ...
    clearCart() {
     ...
     async generateInvoiceNumber() {
         const seq = await this.db.getNextSequence('invoice');
         return `INV-${String(seq).padStart(5, '0')}`;
     }

     async generatePurchaseNumber() {
         const seq = await this.db.getNextSequence('purchase');
         return `PUR-${String(seq).padStart(5, '0')}`;
     }

     async generateVoucherNumber() {
         const seq = await this.db.getNextSequence('voucher');
         return `VCH-${String(seq).padStart(5, '0')}`;
     }
             ...
             for (const item of this.cart) {
                 ...
                 if (product) {
                     ...
                     const movement = {
                         ...
                         type: 'sale_out',
                         quantity: item.quantity * getUnitMultiplier(item.unit),
                         reference_type: 'invoice',
                         reference_id: prep.invoice.invoice_number,
                         notes: `بيع - ${product.name}`,
                         created_at: now,
                         unit_cost: product.cost_price
                     };
                     await this.db.add('stock_movements', movement);
                 }
             }
...
<p>شكراً لتعاملكم معنا</p>
    ...
    async printPurchaseInvoice(purchaseId) {
        const purchase = await this.db.get('purchases', purchaseId);
        if (!purchase) {
            this.showToast('الفاتورة غير موجودة', 'error');
            return;
        }
        const storeName = await this.getSetting('store_name', 'متجري');
        const phone = await this.getSetting('store_phone', '');
        ...
        const supplierName = supplierMap.get(purchase.supplier_id) || 'مورد محذوف';
        const logoImg = logo ? `<img src="${logo}" style="max-height:50px;max-width:100px;margin-bottom:10px;">` : '';
...
<div class="info">
<p>فاتورة مشتريات رقم: #${escapeHtml(purchase.purchase_number)}</p>
<p>المورد: ${escapeHtml(supplierName)}</p>
<p>التاريخ: ${new Date(purchase.date || purchase.created_at).toLocaleString('ar-SA')}</p>
</div>
<table>
<thead><tr><th>الصنف</th><th>الكمية</th><th>السعر</th><th>الإجمالي</th></tr></thead>
<tbody>
${(purchase.items || []).map(item => `<tr><td>${escapeHtml(item.product_name)}</td><td>${item.quantity} ${getUnitName(item.unit || 'box')}</td><td>${this.formatCurrency(item.unit_cost_input) || (item.unit_cost ? item.unit_cost * getUnitMultiplier(item.unit || 'box') : item.cost_price || item.price)}</td><td>${this.formatCurrency(item.line_total || (item.unit_cost_input || (item.unit_cost ? item.unit_cost * getUnitMultiplier(item.unit || 'box') : item.cost_price || item.price)) * item.quantity)}</td></tr>`).join('')}
</tbody>
</table>
<div class="total">
<div class="total-row"><span>الإجمالي:</span><span>${this.formatCurrency(purchase.total)}</span></div>
<div class="total-row"><span>المدفوع:</span><span>${this.formatCurrency(purchase.paid_amount || 0)}</span></div>
<div class="total-row"><span>المتبقي:</span><span>${this.formatCurrency((purchase.total || 0) - (purchase.paid_amount || 0))}</span></div>
</div>
<div class="footer">
...
<p>تاريخ الطباعة: ${new Date().toLocaleString('ar-SA')}</p>
    ...
    renderInventoryTable(products) {
        ...
        tbody.innerHTML = products.map((p) => {
            ...
            const delBtn = this.currentUser.role === 'admin'
                ...
                : '';
            const units = convertStock(p.stock);
            const stockDisplay = `كرتون: ${units.boxes} | عروسة: ${units.packs} | حبة: ${units.pieces}`;
            return `<tr>
                <td>${escapeHtml(p.name)}</td>
                <td>${escapeHtml(p.barcode || '-')}</td>
                <td><span class="badge ${p.stock <= p.min_stock ? 'badge-warning' : 'badge-success'}">${stockDisplay}</span></td>
                ${costCol}
    ...
    async loadCustomers(tbodyId = 'accounting-customers-tbody') {
    ...
    async saveCustomer() {
     ...
     // ========== PURCHASE MANAGEMENT ==========
      ...
      async openPurchaseModal(purchaseId = null) {
          const modal = document.getElementById('purchase-modal');
          const form = document.getElementById('purchase-form');
          if (form) form.reset();
          document.getElementById('purchase-id').value = '';
          document.getElementById('purchase-date').value = new Date().toISOString().split('T')[0];
          document.getElementById('purchase-total').value = '0';
        
          // Populate supplier select
          await this.populatePurchaseSupplierSelect();
          ...
          if (purchaseId) {
              ...
              document.getElementById('purchase-date').value = purchase.date || purchase.created_at.split('T')[0];
              ...
              if (purchase.items && purchase.items.length > 0) {
                  for (const item of purchase.items) {
                      await this.addPurchaseItemRow(item.product_id, item.quantity, item.unit, item.unit_cost_input || (item.unit_cost * getUnitMultiplier(item.unit || 'box')));
                  }
              }
          } else {
              // New purchase
              await this.addPurchaseItemRow();
          }
     ...
     closePurchaseModal() {
      ...
      async populatePurchaseSupplierSelect() {
          const supSelect = document.getElementById('purchase-supplier');
          if (!supSelect) return;
          const suppliers = await this.db.getAll('suppliers');
          const current = supSelect.value;
          supSelect.innerHTML = '<option value="">-- شراء نقدي مباشر --</option>' +
              suppliers.map(s => `<option value="${s.id}">${escapeHtml(s.name)}</option>`).join('');
          supSelect.value = current;
      }

      async addPurchaseItemRow(productId = null, qty = 1, unit = 'box', unitCost = 0) {
          const container = document.getElementById('purchase-items-container');
          const products = await this.getProducts();
          const options = products.map(p => `<option value="${p.id}" data-cost="${p.cost_price}" data-wholesale="${p.wholesale_price || 0}" data-box="${p.box_price || 0}">${escapeHtml(p.name)}</option>`).join('');
          const unitOptions = `
              <option value="box" ${unit === 'box' ? 'selected' : ''}>كرتون</option>
              <option value="pack" ${unit === 'pack' ? 'selected' : ''}>عروسة</option>
              <option value="piece" ${unit === 'piece' ? 'selected' : ''}>حبة</option>
          `;
        
          const row = document.createElement('div');
          row.className = 'purchase-item-row';
          row.style = 'display: grid; grid-template-columns: 2fr 1fr 1fr 1fr 40px; gap: 8px; margin-bottom: 8px;';
          row.innerHTML = `
              <select class="purchase-product-select" ${productId ? '' : 'required'} style="padding: 8px;">${options}</select>
              <input type="number" class="purchase-qty" placeholder="الكمية" min="1" value="${qty}" ${productId ? '' : 'required'} style="padding: 8px;">
              <select class="purchase-unit" style="padding: 8px;">${unitOptions}</select>
              <input type="number" class="purchase-unit-cost" placeholder="سعر الوحدة" min="0" step="any" value="${unitCost}" ${productId ? '' : 'required'} style="padding: 8px;">
              <button type="button" class="btn btn-sm btn-danger remove-purchase-item" style="padding: 8px;"><i class="fas fa-times"></i></button>
          `;
          
          if (productId) {
              row.querySelector('.purchase-product-select').value = productId;
          }
          ...
          const updateCost = () => {
              ...
              let cost = parseFloat(chosenOption.dataset.cost) || 0;
              if (unit === 'box') cost = parseFloat(chosenOption.dataset.box) || cost * 500;
              else if (unit === 'pack') cost = parseFloat(chosenOption.dataset.wholesale) || cost * 10;
              row.querySelector('.purchase-unit-cost').value = cost || '';
              this.calculatePurchaseTotal();
          };
          
          row.querySelector('.purchase-product-select').addEventListener('change', updateCost);
          row.querySelector('.purchase-unit').addEventListener('change', updateCost);
          row.querySelectorAll('input').forEach(input => {
              input.addEventListener('input', () => this.calculatePurchaseTotal());
          });
          
          if (!productId) updateCost();
      }
     ...
     calculatePurchaseTotal() {
         ...
         rows.forEach(row => {
             ...
             const unitCost = parseFloat(row.querySelector('.purchase-unit-cost').value) || 0;
             total += qty * unitCost;
         });
       ...
       async savePurchase() {
          const editId = document.getElementById('purchase-id').value;
          const isEdit = !!editId;
        
          const supplierIdValue = document.getElementById('purchase-supplier').value;
         ...
         // Collect items
          const rows = document.querySelectorAll('.purchase-item-row');
          if (rows.length === 0) {
              this.showToast('أضف منتج واحد على الأقل', 'error');
              return;
          }
          ...
          for (const row of rows) {
              const productId = parseInt(row.querySelector('.purchase-product-select').value);
              const qty = parseInt(row.querySelector('.purchase-qty').value);
              const unit = row.querySelector('.purchase-unit')?.value || 'box';
              const unitCost = parseFloat(row.querySelector('.purchase-unit-cost').value);
              if (productId && qty > 0 && unitCost >= 0) {
                  const product = await this.db.get('products', productId);
                  if (product) {
                      const multiplier = getUnitMultiplier(unit);
                      const qtyInPieces = qty * multiplier;
                      const costPerPiece = unitCost / multiplier;
                      items.push({
                          product_id: productId,
                          product_name: product.name,
          ...
          if (items.length === 0) {
              this.showToast('أدخل منتجات صالحة', 'error');
              return;
          }
        
          if (paymentMethod === 'credit' && !supplierId) {
              this.showToast('لا يمكن تنفيذ شراء آجل بدون تحديد مورد', 'error');
              return;
          }
          ...
          try {
              ...
              if (isEdit) {
                  oldPurchase = await this.db.get('purchases', parseInt(editId, 10));
                  if (!oldPurchase) {
                      this.showToast('الفاتورة غير موجودة', 'error');
                      return;
                  }
                  purchaseNumber = oldPurchase.purchase_number;
        
                  const allAccountEntries = await this.db.getAll('account_entries');
                  for (const e of allAccountEntries) {
                      if (e.ref_type === 'purchase' && String(e.ref_id) === String(purchaseNumber)) {
                          ops.push({ type: 'delete', store: 'account_entries', id: e.id });
                      }
                  }
        
                  const allMovements = await this.db.getAll('stock_movements');
                  for (const m of allMovements) {
                      if (String(m.reference_id) === String(purchaseNumber)) {
                          ops.push({ type: 'delete', store: 'stock_movements', id: m.id });
                      }
                  }
              } else {
              ...
              const purchase = {
                  ...
                  items,
                  total,
                  paid_amount: paymentMethod === 'cash' ? total : (paymentMethod === 'mixed' ? paidAmount : 0),
                  due_amount: total - (paymentMethod === 'cash' ? total : (paymentMethod === 'mixed' ? paidAmount : 0)),
                  payment_method: paymentMethod,
                  notes,
                  status: 'completed',
                  created_by: isEdit && oldPurchase ? oldPurchase.created_by : this.currentUser.name,
                  created_at: isEdit && oldPurchase ? oldPurchase.created_at : now
              };
        
              if (isEdit) {
                  purchase.id = parseInt(editId, 10);
                  ops.push({ type: 'put', store: 'purchases', value: purchase });
              } else {
                  ops.push({ type: 'add', store: 'purchases', value: purchase });
              }
        
              // Build journal entries for purchases
              const journalId = newJournalId();
              const lines = [];
              ...
              lines.push({
                  ...
                  debit: total,
                  credit: 0,
                  entry_kind: 'purchase_inv',
                  memo: `مشتريات - ${purchaseNumber}`
              });
              ...
              if (paymentMethod === 'cash' || paymentMethod === 'mixed') {
                  ...
                  lines.push({
                      ...
                      debit: 0,
                      credit: paid,
                      entry_kind: 'purchase_cash',
                      memo: `دفع نقدي - ${purchaseNumber}`
                  });
              }
        
              if (supplierId && (paymentMethod === 'credit' || paymentMethod === 'mixed')) {
                  const due = paymentMethod === 'credit' ? total : (total - (paidAmount || 0));
                  lines.push({
                      account_code: AC.AP,
                      debit: 0,
                      credit: due,
                      party_key: `supplier:${supplierId}`,
                      entry_kind: 'purchase_ap',
                      memo: `مستحق للمورد - ${purchaseNumber}`
                  });
              }
        
              assertBalanced(lines);
        
              const accountEntries = lines.map((l) => ({
                  journal_id: journalId,
                  account_code: l.account_code,
                  debit: l.debit,
                  credit: l.credit,
                  party_key: l.party_key,
                  ref_type: 'purchase',
                  ref_id: purchaseNumber,
                  ref_key: `purchase:${purchaseNumber}`,
              ...
              for (const entry of accountEntries) {
                  ops.push({ type: 'add', store: 'account_entries', value: entry });
              }
        
              const balDelta = {};
              if (isEdit && oldPurchase && oldPurchase.supplier_id) {
                  const oldEffect = (Number(oldPurchase.total) || 0) - (Number(oldPurchase.paid_amount) || 0);
                  balDelta[oldPurchase.supplier_id] = (balDelta[oldPurchase.supplier_id] || 0) - oldEffect;
              }
              if (supplierId) {
                  const paymentDue = paymentMethod === 'cash' ? total : (paymentMethod === 'mixed' ? paidAmount : 0);
                  const newEffect = total - paymentDue;
                  balDelta[supplierId] = (balDelta[supplierId] || 0) + newEffect;
              }
              for (const [sid, delta] of Object.entries(balDelta)) {
                  const s = await this.db.get('suppliers', parseInt(sid, 10));
                  if (s) {
                      ops.push({ type: 'put', store: 'suppliers', value: { ...s, balance: (s.balance || 0) + delta } });
                  }
              }
        
              // Stock & WAC: remove old line effects in memory, then add new (edit-safe)
              const productIds = new Set();
              if (isEdit && oldPurchase) {
                  for (const it of oldPurchase.items || []) productIds.add(it.product_id);
              }
              for (const it of items) productIds.add(it.product_id);
              for (const productId of productIds) {
                  let p = await this.db.get('products', productId);
                  if (!p) continue;
                  if (isEdit && oldPurchase) {
                      for (const it of (oldPurchase.items || []).filter((x) => x.product_id === productId)) {
                          p = applyPurchaseLineReverseToProduct(p, it);
                      }
                  }
                  for (const it of items.filter((x) => x.product_id === productId)) {
                      p = applyPurchaseLineAddToProduct(p, it);
                  }
                  ops.push({ type: 'put', store: 'products', value: p });
              }
        
              const supplier = supplierId ? await this.db.get('suppliers', supplierId) : null;
              for (const item of items) {
                  ops.push({
                      ...
                      value: {
                          ...
                          notes: `شراء من ${supplier ? supplier.name : 'نقدي مباشر'}`,
                          created_at: now,
              ...
              if (isEdit && oldPurchase && oldPurchase.supplier_id) {
                  await this.syncPartyBalanceFromGl('supplier', oldPurchase.supplier_id);
              }
              if (supplierId) {
                  await this.syncPartyBalanceFromGl('supplier', supplierId);
              }
        
              this.logActivity('فاتورة مشتريات', `تم ${isEdit ? 'تعديل' : 'إنشاء'} فاتورة مشتريات ${purchaseNumber} بقيمة ${this.formatCurrency(total)}`);
              this.closePurchaseModal();
              await this.loadPurchases();
              this.invalidateCache();
              this.showToast(`تم ${isEdit ? 'تعديل' : 'حفظ'} فاتورة المشتريات بنجاح`, 'success');
          } catch (e) {
              console.error('Save purchase error:', e);
              this.showToast('خطأ في حفظ الفاتورة: ' + (e && e.message ? e.message : e), 'error');
          }
      }
      ...
      async loadPurchases() {
         try {
             ...
             const suppliers = await this.db.getAll('suppliers');
             const supplierMap = new Map(suppliers.map(s => [s.id, s.name]));
             this.renderPurchasesTable(purchases, supplierMap);
         } catch (e) {
             console.error('Purchases load error:', e);
         }
     }

      renderPurchasesTable(purchases, supplierMap) {
          ...
          if (sorted.length === 0) {
              tbody.innerHTML = '<tr><td colspan="7" class="empty-state">لا توجد فواتير مشتريات</td></tr>';
              return;
          }
          tbody.innerHTML = sorted.map((p) => {
              const date = new Date(p.date || p.created_at).toLocaleDateString('ar-SA');
              const supplierName = p.supplier_id ? (supplierMap.get(p.supplier_id) || 'مورد محذوف') : 'شراء نقدي مباشر';
              const statusBadge = p.status === 'completed'
                  ? '<span class="badge badge-success">مكتملة</span>'
                  : '<span class="badge badge-danger">ملغاة</span>';
              const editBtn = (p.status === 'completed' && (this.currentUser.role === ROLES.ADMIN || this.currentUser.role === ROLES.DEVELOPER))
                  ? `<button type="button" class="btn btn-sm btn-outline" data-purchase-action="edit" data-id="${p.id}" style="margin-right:4px;"><i class="fas fa-edit"></i> تعديل</button>`
                  : '';
              const retBtn = (p.status === 'completed' && (this.currentUser.role === ROLES.ADMIN || this.currentUser.role === ROLES.DEVELOPER))
                  ? `<button type="button" class="btn btn-sm btn-outline" data-purchase-action="return" data-id="${p.id}" data-number="${p.purchase_number}" style="margin-right:4px;"><i class="fas fa-undo"></i> مرتجع</button>`
                  : '';
              const printBtn = `<button type="button" class="btn btn-sm btn-outline" data-purchase-action="print" data-id="${p.id}" style="margin-right:4px;"><elem style="fas fa-print"></i> طباعة</button>`;
              return `<tr>
                  <td>#${escapeHtml(p.purchase_number)}</td>
                  <td>${escapeHtml(supplierName)}</td>
                  <td>${date}</td>
                  <td>${this.formatCurrency(p.total)}</td>
                  <td>${this.formatCurrency(p.paid_amount || 0)}</td>
                  <td>${statusBadge}</td>
                  <td>
                      ${editBtn}
                      ${printBtn}
     ...
     async handlePurchaseReturn(purchaseId, purchaseNumber) {
         const purchase = await this.db.get('purchases', purchaseId);
         if (!purchase) {
             this.showToast('الفاتورة غير موجودة', 'error');
             return;
         ...
         if (purchase.status !== 'completed') {
             this.showToast('يمكن فقط فواتير مكتملة', 'error');
             return;
         }
         const pnum = purchase.purchase_number || purchaseNumber;
         if (!confirm(`هل أنت متأكد من إلغاء فاتورة المشتريات ${pnum}؟ سيتم عكس المخزون والحسابات.`)) return;
         ...
         try {
             ...
             for (const item of purchase.items) {
                 ...
                 if (product) {
                     ...
                     const newStock = product.stock - returnedPieces;
                     if (newStock < 0) {
                         this.showToast(`مخزون سالب غير مسموح: ${product.name}`, 'error');
                         return;
                     }
                     const oldTotalCost = product.cost_price * product.stock;
                     const returnedCost = item.unit_cost * returnedPieces;
                     const newTotalQty = product.stock - returnedPieces;
                     const newWacCost = newTotalQty > 0 ? (oldTotalCost - returnedCost) / newTotalQty : product.cost_price;
                     ops.push({ type: 'put', store: 'products', value: { ...product, stock: newStock, cost_price: newWacCost } });
                 }
             }
       
             ops.push({ type: 'put', store: 'purchases', value: { ...purchase, status: 'cancelled' } });
       
             const allGl = await this.db.getAll('account_entries');
             const entries = allGl.filter(
                 (e) => e.ref_type === 'purchase' && String(e.ref_id) === String(pnum)
             );
             const journalId = newJournalId();
             let reversalRows = [];
             if (entries.length > 0) {
                 try {
                     reversalRows = buildReversalJournal(
                         journalId,
                         entries,
                         now,
                         `إلغاء فاتورة مشتريات ${pnum}`
                     );
                 } catch (jErr) {
                     console.warn('GL reversal from stored entries failed, using computed reversal:', jErr);
                     reversalRows = buildPurchaseReversalFromInvoiceData(purchase, newJournalId(), now);
                 }
             } else {
                 reversalRows = buildPurchaseReversalFromInvoiceData(purchase, journalId, now);
             }
             for (const entry of reversalRows) {
                 ops.push({ type: 'add', store: 'account_entries', value: entry });
             }
             ...
             if (purchase.supplier_id) {
                 await this.syncPartyBalanceFromGl('supplier', purchase.supplier_id);
             }
       
             this.logActivity('مرتجع مشتريات', `تم إلغاء فاتورة مشتريات ${pnum}`);
             await this.loadPurchases();
             this.invalidateCache();
             this.showToast('تم إلغاء الفاتورة بنجاح', 'success');
         } catch (e) {
             console.error('handlePurchaseReturn error:', e);
             this.showToast('فشل تنفيذ المرتجع: ' + (e && e.message ? e.message : e), 'error');
         }
     }
    ...
    refreshReportFilters() {
      ...
      async showReport(type) {
          ...
          const mvEl = document.getElementById('movements-report');
          if (mvEl) mvEl.style.display = type === 'movements' ? 'block' : 'none';
         ...
         else if (type === 'movements') await this.showStockMovementsReport(from, to);
     }
    ...
    async showProductsReport(from, to) {
        ...
        document.getElementById('products-report').innerHTML = `
            <h3 style="margin-bottom: 20px;">تقرير المنتجات (كرتون/عروسة/حبة)</h3>
            ${sorted.length === 0 ? '<p>لا توجد بيانات</p>' : `<table class="table"><thead><tr><th>المنتج</th><th>الكمية (كرتون/عروسة/حبة)</th><th>الإجمالي</th></tr></thead><tbody>${
                sorted.map((p) => {
                    const su = convertStock(p.pieces);
                    const display = `كرتون: ${su.boxes} | عروسة: ${su.packs} | حبة: ${su.pieces}`;
                    return `<tr><td>${escapeHtml(p.name)}</td><td>${display}</td><td>${this.formatCurrency(p.total)}</td></tr>`;
                }).join('')
            }</tbody></table>`}`;
    }
    ...
    async showBalanceSheetReport() {
     ...
     async showStockMovementsReport(from, to) {
         const movements = await this.db.getAll('stock_movements');
         const products = await this.getProducts();
         const productMap = new Map(products.map(p => [p.id, p.name]));
       
         const filtered = movements.filter(m => this._inDateRange(m.created_at, from, to));
         const sorted = filtered.sort((a, b) => new Date(b.created_at) - new Date(a.created_at));
       
         const typeLabels = {
             'sale_out': 'بيع (خرج)',
             'purchase_in': 'شراء (دخل)',
             'transfer': 'تحويل',
             'adjustment': 'تعديل'
         };
       
         document.getElementById('movements-report').innerHTML = `
             <h3 style="margin-bottom: 12px;">حركة المخزون</h3>
             <p style="margin-bottom: 20px; color: var(--gray); font-size: 14px;">
                 حركة المنتجات من/${from || 'البداية'} إلى/${to || 'الآن'}
             </p>
             <table class="table">
                  <thead>
                      <tr>
                  ...
                  <tbody>
                      ${sorted.map((m, i) => {
                          const su = convertStock(m.quantity);
                          const stockDisplay = `كرتون: ${su.boxes} | عروسة: ${su.packs} | حبة: ${su.pieces}`;
                          return `
                          <tr>
                              <td>${i + 1}</td>
                              <td>${escapeHtml(productMap.get(m.product_id) || 'منتج محذوف')}</td>
                              <td><span class="badge ${m.type.includes('in') ? 'badge-success' : (m.type.includes('out') ? 'badge-danger' : 'badge-info')}">${escapeHtml(typeLabels[m.type] || m.type)}</span></td>
                              <td>${stockDisplay}</td>
                              <td>
                                  ${m.unit_cost ? `<span class="badge">${this.formatCurrency(m.unit_cost)}/حبة</span>` : ''}
                              </td>
             ...
             <div style="margin-top: 16px; text-align: center;">
                 <button type="button" class="btn btn-outline report-print-btn"><i class="fas fa-print"></i> طباعة التقرير</button>
             </div>
         `;
     }
    ...
    async showLogoInHeader() {
     ...
     async loadSettingsScreen() {
         ...
         const dc = document.getElementById('dev-tools-card');
         ...
         if (dc) dc.style.display = this.currentUser?.role === ROLES.DEVELOPER ? 'block' : 'none';
    ...
    async loadSuppliers() {
     ...
     openSupplierModal(s = null) {
         ...
         if (s) {
             ...
             document.getElementById('supplier-balance').value = s.balance || 0;
         } else {
             if (title) title.textContent = 'إضافة مورد';
             document.getElementById('supplier-form').reset();
             document.getElementById('supplier-id').value = '';
             document.getElementById('supplier-balance').value = 0;
         }
    ...
    async deleteSupplier(id) {
     ...
     async saveSupplier() {
         ...
         const openingBalance = parseFloat(document.getElementById('supplier-balance').value) || 0;
         const data = {
         ...
         if (id) {
             ...
             data.id = parseInt(id, 10);
             // Keep existing balance; sync from GL will correct if needed
             data.balance = prev.balance || 0;
             ...
             // Create opening balance GL entry if > 0
             if (openingBalance > 0) {
                 const now = new Date().toISOString();
                 const journalId = newJournalId();
                 const openingEntries = [
                     {
                         ...
                         debit: 0,
                         credit: openingBalance,
                         party_key: `supplier:${newId}`,
                         ref_type: 'opening_balance',
                         ref_id: 'OPENING',
                         ref_key: `supplier_opening_${newId}`,
                         entry_kind: 'opening_ap',
                         memo: `رصيد افتتاحي للمورد ${data.name}`,
                         created_at: now
                     },
                     {
                         journal_id: journalId,
                         ...
                         ref_type: 'opening_balance',
                         ref_id: 'OPENING',
                         ref_key: `supplier_opening_cash_${newId}`,
                         entry_kind: 'opening_cash_adjust',
                         memo: `تعديل النقدية لرصيد افتتاحي`,
                         created_at: now
                     }
                 ];
                 await this.db.executeWrites(openingEntries.map(e => ({ type: 'add', store: 'account_entries', value: e })));
    ...
    async openVoucherModal(type) {
        ...
        const hintEl = document.getElementById('voucher-modal-hint');
        if (titleEl) {
            titleEl.textContent = type === 'receipt' ? 'سند قبض نقدي' : 'سند صرف نقدي';
        }
        if (hintEl) {
            hintEl.textContent =
                type === 'receipt'
                    ? 'القيد: مدين النقدية، دائن ذمم العميل (AR) أو ذمم المورد (AP) حسب نوع الطرف المختار.'
                    : 'القيد: مدين ذمم المورد (AP) أو ذمم العميل (AR)، دائن النقدية — حسب نوع الطرف المختار.';
        }
    ...
    async saveVoucher() {
        ...
        const glRows =
            type === 'receipt'
                ...
                : buildPaymentVoucherJournal(journalId, {
                    amount,
                    partyId,
                    partyType,
                    voucherNumber: num,
        ...
        await this.db.executeWrites(ops);
        await this.syncPartyBalanceFromGl(partyType, partyId);
    ...
    async loadVouchersList() {
        ...
        if (!el) return;
        if (!list.length) {
            el.innerHTML = '<p class="empty-state">لا سندات</p>';
            return;
        }
        const customers = await this.db.getAll('customers');
        const suppliers = await this.db.getAll('suppliers');
        ...
        const partyLabel = (v) => {
            const nm =
                v.party_type === 'customer' ? cMap.get(v.party_id) : sMap.get(v.party_id);
            const t = v.party_type === 'customer' ? 'عميل' : 'مورد';
            return `${t}: ${escapeHtml(nm || `#${v.party_id}`)}`;
        };
        const entryDesc = (v) => {
            if (v.voucher_type === 'receipt') {
                return v.party_type === 'customer'
                    ? 'قبض — دائن AR (تسوية ذمة عميل)'
                    : 'قبض — دائن AP (تسوية ذمة مورد)';
            }
            return v.party_type === 'supplier'
                ? 'صرف — مدين AP (سداد لمورد)'
                : 'صرف — مدين AR (تسوية مع عميل)';
        };
        el.innerHTML = `<table class="table"><thead><tr><th>الرقم</th><th>النوع</th><th>الطرف</th><th>القيد</th><th>المبلغ</th><th>طباعة</th></tr></thead><tbody>${list
            .map(
                (v) => `<tr>
                <td>${escapeHtml(v.voucher_number)}</td>
                <td>${v.voucher_type === 'receipt' ? 'قبض نقدي' : 'صرف نقدي'}</td>
                <td>${partyLabel(v)}</td>
                <td style="font-size:12px;color:var(--gray);">${entryDesc(v)}</td>
    ...
    async printVoucherHtml(v) {
        ...
        const w = window.open('', '_blank');
        const title = v.voucher_type === 'receipt' ? 'سند قبض نقدي' : 'سند صرف نقدي';
        const html = `<!DOCTYPE html>
<html lang="ar" dir="rtl">
<head>
<meta charset="UTF-8">
    ...
    async runAccountStatement() {
        ...
        out.innerHTML = `<h4>كشف حساب: ${escapeHtml(partyName)}</h4><table class="table"><thead><tr><th>التاريخ</th><th>البيان</th><th>مدين</th><th>دائن</th><th>الرصيد</th></tr></thead><tbody>${
            entries.map((e) => {
                ...
                const dStr = new Date(e.created_at).toLocaleDateString('ar-SA');
                return `<tr><td>${dStr}</td><td>${escapeHtml(e.memo || '')}</td><td>${dr}</td><td>${cr}</td><td>${this.formatCurrency(bal)}</td></tr>`;
            }).join('')
        }</tbody></table>`;
    }

    /** ترويسة وطباعة بنفس أسلوب كشف الحساب (للتقارير والكشوف). */
    async buildStandardPrintDocumentHtml(documentTitle, mainHeading, subLine, bodyInnerHtml) {
        const storeName = await this.getSetting('store_name', 'متجري');
        const phone = await this.getSetting('store_phone', '');
        const logo = await this.getSetting('store_logo', '');
...
.report-body :where(h3,h4) { text-align: center; margin: 10px 0 8px; }
.report-body .grid { display: block; }
.report-body .card { border: 1px solid #ccc; padding: 8px; margin: 8px 0; page-break-inside: avoid; }
.report-body .stat-value { font-weight: 700; }
...
${subLine ? `<p style="text-align:center;color:#555;margin:4px 0 8px;font-size:11px;">${subLine}</p>` : ''}
${mainHeading ? `<h3 style="text-align:center;margin:10px 0;">${escapeHtml(mainHeading)}</h3>` : ''}
<div class="report-body">${bodyInnerHtml}</div>
<div class="footer">
<p>شكراً لتعاملكم معنا</p>
<p>تاريخ الطباعة: ${new Date().toLocaleString('ar-SA')}</p>
</div>
    ...
    openPrintDocument(html) {
        const w = window.open('', '_blank');
        if (!w) {
            this.showToast('تعذر فتح نافذة الطباعة', 'error');
            return;
        }
        w.document.write(html);
        w.document.close();
    ...
    async printActiveReportWindow() {
        const active = document.querySelector('#reports-screen [data-report].active')?.dataset.report || 'daily';
        const map = {
            daily: 'daily-report',
        ...
        const elId = map[active];
        const el = elId ? document.getElementById(elId) : null;
        if (!el || el.style.display === 'none') {
            this.showToast('اختر تقريراً ثم اعرضه قبل الطباعة', 'warning');
            return;
        }
        const titles = {
            daily: 'تقرير فترة (يومي)',
        ...
        const from = document.getElementById('report-date-from')?.value || '';
        const to = document.getElementById('report-date-to')?.value || '';
        const period = `من تاريخ: ${escapeHtml(from || '—')} — إلى: ${escapeHtml(to || '—')}`;
        let inner = el.innerHTML;
        inner = inner.replace(/<button[^>]*class="[^"]*report-print-btn[^"]*"[^>]*>[\s\S]*?<\/button>/gi, '');
        const html = await this.buildStandardPrintDocumentHtml(
            titles[active] || 'تقرير',
            '',
            period,
            inner
        );
        this.openPrintDocument(html);
    }
    ...
    async printStatement() {
        ...
        let bal = 0;
        const rows = this.lastStatement.entries
            .map((e) => {
                const delta = entryPartyRunningDelta(e);
                bal += delta;
                const dr = e.debit ? this.formatCurrency(e.debit) : '—';
                const cr = e.credit ? this.formatCurrency(e.credit) : '—';
                return `<tr><td>${new Date(e.created_at).toLocaleDateString('ar-SA')}</td><td>${escapeHtml(e.memo || '')}</td><td>${dr}</td><td>${cr}</td><td>${this.formatCurrency(bal)}</td></tr>`;
            })
            .join('');
...
<tbody>${rows}</tbody>
    ...
    async importBackup(file) {
        ...
        try {
            for (const store of Object.keys(data)) {
                ...
                for (const row of data[store]) {
                    try {
                        ...
                        console.error(`Failed to import row in ${store}:`, row, rowError);
                    }
                }
            }
            this.invalidateCache();
            ...
            this.showToast('فشل استعادة النسخة: ' + (e.message || e), 'error');
        }
    }
    ...
    async revertEntry(entryId) {
     ...
     async resetSystem() {
         if (!confirm('هل أنت متأكد من إعادة تعيين النظام؟ سيتم حذف جميع البيانات ولا يمكن التراجع.')) return;
         if (!confirm('تحذير نهائي: سيتم حذف جميع الفواتير والعملاء والموردين والمنتجات والسجل. هل تريد المتابعة؟')) return;
       
         try {
             // Delete all transactional stores (keep users, settings, meta)
             const stores = ['products', 'invoices', 'purchases', 'customers', 'suppliers', 'expenses', 'account_entries', 'vouchers', 'stock_movements', 'activity'];
             for (const store of stores) {
             ...
             // Reset sequences
             const seqPrefixes = ['invoice', 'purchase', 'voucher'];
             for (const prefix of seqPrefixes) {
                 await this.db.setSequence(prefix, 0);
             }
           
             // Re-seed default data and ensure opening balances
             await this.seedDataIfNeeded();
             await this.ensureOpeningBalanceEntry();
             await this.loadSettings();
             this.invalidateCache();
