...
/**
 * سند قبض نقدي: مدين النقدية، دائن ذمم العميل (AR) أو ذمم المورد (AP) حسب نوع الطرف.
 * @param {{ amount: number, partyId: number, partyType?: 'customer'|'supplier', voucherNumber: string, memo?: string, now: string }} p
 */
export function buildReceiptVoucherJournal(journalId, p) {
    const { amount, partyId, voucherNumber, memo, now, partyType = 'customer' } = p;
            ...
            ? [
                  {
                      ...
                      memo: memo || `سند قبض نقدي ${voucherNumber} — مقابل ذمة مورد`
                  },
                  {
                      account_code: AC.AP,
                      debit: 0,
                      ...
                      entry_kind: 'receipt_ap',
                      memo: memo || `سند قبض نقدي ${voucherNumber} — تسوية ذمة مورد`
                  }
              ]
            : [
                  {
                      ...
                      credit: 0,
                      entry_kind: 'receipt_cash',
                      memo: memo || `سند قبض نقدي ${voucherNumber} — مقابل ذمة عميل`
                  },
                  {
                      account_code: AC.AR,
                      debit: 0,
                      ...
                      memo: memo || `سند قبض نقدي ${voucherNumber} — تسوية ذمة عميل`
                  }
              ];
    assertBalanced(lines);
...
/**
 * سند صرف نقدي: للمورد مدين AP دائن نقدية؛ للعميل مدين AR (تسوية ذمة) دائن نقدية.
 * @param {{ amount: number, partyId: number, partyType?: 'customer'|'supplier', voucherNumber: string, memo?: string, now: string }} p
 */
export function buildPaymentVoucherJournal(journalId, p) {
    const { partyType = 'supplier' } = p;
    if (partyType === 'supplier') {
        return buildSupplierPaymentJournal(journalId, p);
    }
    const { amount, partyId, voucherNumber, memo, now } = p;
    const refKey = `voucher:${voucherNumber}`;
    const lines = [
        {
            ...
            credit: 0,
            party_key: `customer:${partyId}`,
            entry_kind: 'payment_ar',
            memo: memo || `سند صرف نقدي ${voucherNumber} — تسوية ذمة عميل`
        },
        {
            account_code: AC.CASH,
            debit: 0,
            credit: amount,
            entry_kind: 'payment_cash',
            memo: memo || `سند صرف نقدي ${voucherNumber}`
        }
    ];
    assertBalanced(lines);
    return journalLinesToAccountEntries(journalId, lines, 'voucher', voucherNumber, refKey, now);
